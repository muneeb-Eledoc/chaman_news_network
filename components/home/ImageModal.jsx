...
const ImageModal = ({imageToOpen, setImageToOpen}) => {
  ...
  return (
    <div onClick={closeModal} className={`w-full h-screen fixed flex justify-center items-center top-0 left-0 bg-black z-40 transition-all duration-300 ease-in-out ${imageToOpen ? 'scale-100 opacity-100' : 'scale-0 opacity-0'}`}>
        ...
        <img className='max-w-[100%] max-h-screen z-50' src={imageToOpen} alt="" />
    </div>
  )
}
